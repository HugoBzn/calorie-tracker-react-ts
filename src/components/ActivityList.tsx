        ...
        activities.map((activity) => (
          <div key={activity.id} className="px-5 py-10 bg-white flex justify-between shadow">
            <div className="space-y-2 relative">
              <p
                className={`absolute -top-8 -left-8 py-2 px-10 text-white uppercase font-bold ${activity.category === 1 ? "bg-lime-500" : "bg-orange-500"}`}
              >
            ...
            <div className="flex gap-5 items-center">
              <button onClick={() => dispatch({ type: "set-activeId", payload: { id: activity.id } })}>
                <PencilSquareIcon className="h-8 w-8 text-gray-800 hover:cursor-pointer" />
              </button>

              <button onClick={() => dispatch({ type: "delete-activity", payload: { id: activity.id } })}>
                <XCircleIcon className="h-8 w-8 text-red-500 hover:cursor-pointer" />
              </button>
            </div>
          </div>
        ))
